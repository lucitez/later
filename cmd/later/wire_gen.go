...
func InitializeShare(db *sql.DB) server.ShareServer {
	...
	user := repository.NewUser(db)
	serviceUser := service.NewUser(user)
	notification := service.NewNotification(serviceUser)
	serviceUserContent := service.NewUserContent(userContent, notification)
	...
	serviceUserMessage := service.NewUserMessage(userMessage, serviceUser, notification, serviceChat)
